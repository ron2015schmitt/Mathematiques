...
namespace mathq {
  ...
  /********************************************************************
   * MatrixHermitian<Num>    -- variable size matrix (valarray)
   *                              Num  = type for elements
   * MatrixHermitian<Num,N>  -- fixed number of rows (valarray)
   *                              N = number of rows
   *                                = number of cols
   ********************************************************************
   */
  ...
  class MatrixHermitian : public ExpressionRW<MatrixHermitian<Num, N>, Num, Num, 1, 2> {
  ...
  public:
    ...
    typedef MatrixHermitian<Num, N> ConcreteType;
  ...
  public:
    ...
    // -------------------  DEFAULT  CONSTRUCTOR --------------------
    explicit MatrixHermitian<Num, N>() {
      resize(N);
      *this = 0;
    }

    // -------------------  Num value --------------------
    explicit MatrixHermitian<Num, N>(const Num& value) {
      resize(N);
      *this = value;
    }
    ...
    template<size_t NN = N, EnableIf<(NN > 0)> = 0>
      ...
      explicit MatrixHermitian<Num, N>(const Vector<Num>& v) {
    ...
    template<class X, size_t NN = N, EnableIf<(NN>0)> = 0>
      ...
      explicit MatrixHermitian<Num, N>(const ExpressionR<X, Num, Num, 1, 2> A) {
    ...
    explicit MatrixHermitian<Num, N>(const size_t depth) {
      resize(depth);
      *this = 0;
    }
    ...
    explicit MatrixHermitian<Num, N>(const size_t depth, const Num& value) {
      resize(depth);
      *this = value;
    }
    ...
    MatrixHermitian<Num, N>& resize(const int depth) {
      N_ = N;
    ...
    MatrixHermitian<Num, N>& resize(const Dimensions dims) {
    ...
    MatrixHermitian<Num, N>& resize(const std::vector<Dimensions>& deepdims_new) {
    ...
    MatrixHermitian<Num, N>& transpose(void) {
      // TODO: implement this
      return *this;
    }
    ...
    typename std::enable_if<is_complex<T>::value, MatrixHermitian<Num, N>& >::type adjoint() {
      return *this;
    }
    ...
    MatrixHermitian<Num, N>& set(const Vector<Num>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    }
    MatrixHermitian<Num, N>& operator=(const Vector<Num>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    ...
    template <class X>
    MatrixHermitian<Num, N>& operator=(const ExpressionR<X, Num, Num, 1, 1>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    ...
    template <class X>
    MatrixHermitian<Num, N>& operator=(const ExpressionR<X, Num, Num, 1, 2>& A) {
      const size_t depth = A.Nrows();
    ...
    MatrixHermitian<Num, N>& operator=(const Num& value) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = value;
      }
      return *this;
    ...
    MatrixHermitian<Num, N>& operator=(const MatrixHermitian<Num, N>& b) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = b[k];
      }
      return *this;
    ...
    MatrixHermitian<Num, N>& roundzero(SimpleNumberType tolerance = Functions<SimpleNumberType>::tolerance) {
      return *this;
    }
    ...
    template< typename T = Num >
    typename std::enable_if<is_complex<T>::value, MatrixHermitian<Num, N>& >::type conj() {
      return *this;
    }
    ...
    friend std::ostream& operator<<(std::ostream& stream, const MatrixHermitian<Num, N>& m) {
      using namespace display;
    ...
    friend inline std::istream& operator>>(const std::string s, MatrixHermitian<Num, N>& m2) {
    ...
    friend std::istream& operator>>(std::istream& stream, MatrixHermitian<Num, N>& m2) {
      return stream;
    }
