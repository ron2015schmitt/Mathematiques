...
namespace mathq {
  ...
  class MatrixUpperTriangle : public MArrayExpRW<MatrixUpperTriangle<Number, N>, Number, Number, 1, 2> {
  ...
  public:
    ...
    explicit MatrixUpperTriangle<Number, N>(const MArrayExpR<X, Number, Number, 1, 2> A) {
    ...
    explicit MatrixUpperTriangle<Number, N>(const size_t M) {
      resize(M);
      *this = 0;
    }
    ...
    explicit MatrixUpperTriangle<Number, N>(const size_t M, const Number& value) {
      resize(M);
      *this = value;
    }
    ...
    MatrixUpperTriangle<Number, N>& resize(const int M) {
      N_ = N;
    ...
    typename std::enable_if<is_complex<T>{}, MatrixUpperTriangle<Number, N>& >::type adjoint() {
      return *this;
    }
    ...
    template <class X>
    MatrixUpperTriangle<Number, N>& operator=(const MArrayExpR<X, Number, Number, 1, 1>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    ...
    template <class X>
    MatrixUpperTriangle<Number, N>& operator=(const MArrayExpR<X, Number, Number, 1, 2>& A) {
      const size_t M = A.Nrows();
    ...
    template< typename T = Number >
    typename std::enable_if<is_complex<T>{}, MatrixUpperTriangle<Number, N>& >::type conj() {
      return *this;
    }
