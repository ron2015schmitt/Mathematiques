...
namespace mathq {
  ...
  public:
    ...
    typedef typename ArrayType<Number, NC>::Type MyArrayType;
  ...
  public:
    ...
    typename std::enable_if<is_complex<T>{}, MatrixRepRow<Number, NR, NC>& >::type adjoint() {
      return *this;
    }
    ...
    template <class X>
    MatrixRepRow<Number, NR, NC>& operator=(const MArrayExpR<X, Number, Number, 1, 1>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    ...
    template< typename T = Number >
    typename std::enable_if<is_complex<T>{}, MatrixRepRow<Number, NR, NC>& >::type conj() {
      return *this;
    }
