...
namespace mathq {
  ...
  /********************************************************************
   * MatrixToeplitz<Number        -- variable size matrix (valarray)
   *                        Number  = type for elements
   * MatrixToeplitz<Number,NR>    -- fixed number of rows (valarray)
   *                        NR = number of rows
   * MatrixToeplitz<Number,NR,NC> -- fixed number of rows and cols (array)
   *                        NC = number of cols
   ********************************************************************
   */
  ...
  public:
    ...
    typedef MatrixToeplitz<Number, NR, NC> XType;
  ...
  public:
    ...
    MatrixToeplitz<Number, NR, NC>& resize(const int Nr, const int Nc) {
      Nrows_ = NR;
      Ncols_ = NC;
    ...
    MatrixToeplitz<Number, NR, NC>& resize(const Dimensions dims) {
    ...
    MatrixToeplitz<Number, NR, NC>& resize(const std::vector<Dimensions>& deepdims_new) {
    ...
    MatrixToeplitz<Number, NR, NC>& transpose(void) {
      return *this;
    }
    ...
    MatrixToeplitz<Number, NR, NC>& set(const Vector<Number>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    }
    MatrixToeplitz<Number, NR, NC>& operator=(const Vector<Number>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    ...
    MatrixToeplitz<Number, NR, NC>& operator=(const Number& value) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = value;
      }
      return *this;
    ...
    MatrixToeplitz<Number, NR, NC>& operator=(const MatrixToeplitz<Number, NR, NC>& b) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = b[k];
      }
      return *this;
    ...
    MatrixToeplitz<Number, NR, NC>& roundzero(FType tolerance = Functions<FType>::tolerance) {
      return *this;
    }
    ...
    friend std::ostream& operator<<(std::ostream& stream, const MatrixToeplitz<Number, NR, NC>& m) {
      using namespace display;
    ...
    friend inline std::istream& operator>>(const std::string s, MatrixToeplitz<Number, NR, NC>& m2) {
    ...
    friend std::istream& operator>>(std::istream& stream, MatrixToeplitz<Number, NR, NC>& m2) {
      return stream;
    }
