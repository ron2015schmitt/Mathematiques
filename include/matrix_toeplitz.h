#ifndef MATHQ__MATRIX_TOEPLITZ
#define MATHQ__MATRIX_TOEPLITZ 1
...
namespace mathq {
  ...
  /********************************************************************
   * MatrixToeplitz<Num        -- variable size matrix (valarray)
   *                        Num  = type for elements
   * MatrixToeplitz<Num,NR>    -- fixed number of rows (valarray)
   *                        NR = number of rows
   * MatrixToeplitz<Num,NR,NC> -- fixed number of rows and cols (array)
   *                        NC = number of cols
   ********************************************************************
   */
  ...
  class MatrixToeplitz : public ExpressionRW<MatrixToeplitz<Num, NR, NC>, Num, Num, 1, 2> {
  ...
  public:
    ...
    typedef MatrixToeplitz<Num, NR, NC> ConcreteType;
    ...
    typedef typename ArrayTypeTrait<Num, ((NR+NC-1)* (NR>0)* (NC>0))>::Type MyArrayType;
  ...
  public:
    ...
    // -------------------  DEFAULT  CONSTRUCTOR --------------------
    explicit MatrixToeplitz<Num, NR, NC>() {
      size_t NN = NR*NC;
      resize(NR, NC);
      *this = 0;
    }
    ...
    // -------------------  Num value --------------------
    explicit MatrixToeplitz<Num, NR, NC>(const Num& value) {
      size_t NN = NR*NC;
      resize(NR, NC);
      *this = value;
    }
    ...
    template<size_t NN = NR*NC, EnableIf<(NN > 0)> = 0>
      ...
      explicit MatrixToeplitz<Num, NR, NC>(const Vector<Num>& v) {
    ...
    explicit MatrixToeplitz<Num, NR, NC>(const Vector<Num>& v, const size_t Nr, const size_t Nc) {
      const size_t size = v.size();
      // TODO: chekc that size = NR + NC -1
    ...
    explicit MatrixToeplitz<Num, NR, NC>(const size_t Nr, const size_t Nc) {
      resize(Nr, Nc);
      *this = 0;
    }
    ...
    explicit MatrixToeplitz<Num, NR, NC>(const size_t Nr, const size_t Nc, const Num& value) {
      resize(Nr, Nc);
      *this = value;
    }
    ...
    MatrixToeplitz<Num, NR, NC>& resize(const int Nr, const int Nc) {
      Nrows_ = NR;
      Ncols_ = NC;
    ...
    MatrixToeplitz<Num, NR, NC>& resize(const Dimensions dims) {
    ...
    MatrixToeplitz<Num, NR, NC>& resize(const std::vector<Dimensions>& deepdims_new) {
    ...
    MatrixToeplitz<Num, NR, NC>& reshape(const size_t nr, const size_t nc) {
    ...
    MatrixToeplitz<Num, NR, NC>& transpose(void) {
      return *this;
    }
    ...
    typename std::enable_if<is_complex<T>::value, MatrixToeplitz<Num, NR, NC>& >::type adjoint() {
      return *this;
    }
    ...
    MatrixToeplitz<Num, NR, NC>& set(const Vector<Num>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    }
    MatrixToeplitz<Num, NR, NC>& operator=(const Vector<Num>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    ...
    template <class X>
    MatrixToeplitz<Num, NR, NC>& operator=(const ExpressionR<X, Num, Num, 1, 1>& v) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = v[k];
      }
      return *this;
    ...
    MatrixToeplitz<Num, NR, NC>& operator=(const Num& value) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = value;
      }
      return *this;
    ...
    MatrixToeplitz<Num, NR, NC>& operator=(const MatrixToeplitz<Num, NR, NC>& b) {
      for (size_t k = 0; k < data_.size(); k++) {
        data_[k] = b[k];
      }
      return *this;
    ...
    MatrixToeplitz<Num, NR, NC>& roundzero(SimpleNumberType tolerance = Functions<SimpleNumberType>::tolerance) {
      return *this;
    }
    ...
    template< typename T = Num >
    typename std::enable_if<is_complex<T>::value, MatrixToeplitz<Num, NR, NC>& >::type conj() {
      return *this;
    }
    ...
    friend std::ostream& operator<<(std::ostream& stream, const MatrixToeplitz<Num, NR, NC>& m) {
      using namespace display;
    ...
    friend inline std::istream& operator>>(const std::string s, MatrixToeplitz<Num, NR, NC>& m2) {
    ...
    friend std::istream& operator>>(std::istream& stream, MatrixToeplitz<Num, NR, NC>& m2) {
      return stream;
    }
