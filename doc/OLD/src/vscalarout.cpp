...
int main() {
...

  {
    ...
    GMD_CODE_START("C++");
    ECHO(Vector<double> v1(range<double>(1, 4)));
...

  {
    ...
    ECHO(const double tol = 2e-16);
    ECHO(Vector<complex<double> > v1);
    ECHO(v1 = Complex(range<double>(1, 2), 0.));
    ECHO(v1.roundzero(tol));
    ECHO(Vector<complex<double> > v2);
    ECHO(v2 = Complex(range<double>(-1, -2), range<double>(2, 3)));
    ECHO(v2.roundzero(tol));
    GMD_CODE_END();
    CR();
...

  {
    ...
    ECHO(v = exp(linspace<double>(-1, 1, 13)));
    GMD_CODE_END();
    CR();
